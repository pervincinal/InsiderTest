...
import { run, scenarioLevel, scriptedTick } from './util';

/* ---------- fixtures ---------- */

/** `p` and `q` (player) and `e` (enemy) in a triangle: every pair is a lane. */
function threeTowers(): GameState {
  return createState(
    makeLevel({
      towers: [
        { id: 'p', x: 0, y: 0, owner: 'player', units: 10, level: 1 },
        { id: 'q', x: 0, y: 100, owner: 'player', units: 12, level: 1 },
...
describe('cloneState', () => {
  it('is a deep copy of the mutable state: equal to the source, sharing no references (obstacles excepted)', () => {
    const state = threeTowers();
    applyCommand(state, { type: 'link', owner: 'player', from: 'p', to: 'e' });
    run(state, 23); // t = 1150 ms: one unit out, 150 ms on the link timer
    ...
    expect(copy.mines).not.toBe(state.mines);
    expect(copy.roads['e-p']!.points).not.toBe(state.roads['e-p']!.points);
    expect(copy.modifiers).not.toBe(state.modifiers);
    expect(copy.obstacles).toBe(state.obstacles); // static: shared by reference
...
describe('applyContinue', () => {
  ...
  it('reinforcements are capped at the tower capacity (including the player capacity modifier)', () => {
    const base = createState(makeLevel({ towers: [{ id: 'p', x: 0, y: 0, owner: 'player', units: 20 }]}), 1);
    applyContinue(base, { freezeMs: 5000, infantry: 15 });
    expect(base.towers['p']!.units).toBe(25);

    const boosted = createState(makeLevel({ towers: [{ id: 'p', x: 0, y: 0, owner: 'player', units: 20 }]}), 1, {
      productionMul: 1,
      capacityMul: 1.25,
