...
| G6 | Align native version with `package.json` (`versionName` / `versionCode`, bump on every upload) | done (at 0.4.0 / 4 since 2026-09-17 — `npm run version:sync` run by the Publisher, native files untouched by hand) | `npm run version:sync`; bump to 1.0.0 / build 5+ for the store build |
| G7 | Store listing texts EN + AZ (title, short, full description) — monetization wording, rules v2 wording | done (text, `STORE_LISTING.md` §1; rewritten 2026-09-17 for streams / auto-upgrade / four UI languages) | §1.4 holds the ad-free wording if the economy is cut; §7 R2 |
...
| G10 | Phone screenshots (≥ 2, 9:16) EN and AZ | done (Tech Artist retake, `577b894`, 2026-09-17) | `tower-clash/store/screenshots/{en,az}/01..08.png`, 1080×1920, each < 600 KB, rules-v2 renders (title, tutorial, streams in both colours, upgrade, limit hint, fortress, result, shop Upgrades); frame table in `STORE_LISTING.md` §1.3 |
...
| A11 | Screenshots 6.7" 1290×2796 and 6.5" 1284×2778 | done (Tech Artist retake, `577b894`, 2026-09-17) | `tower-clash/store/screenshots/apple-6.7/en/01..09.png` and `apple-6.5/en/01..09.png`, exact device sizes, same eight frames as G10 plus the App-Store-only 09 (level 15, "Silence the guns"); rendered by `renderStoreShots.mjs --apple` |
...
| # | Step | Status | Notes |
|---|---|---|---|
| V1 | `tower-clash/package.json` version `0.4.0`, `config.buildNumber` 4 | done (Publisher, 2026-09-17) | Next: `1.0.0` / 5+ for the first store upload with IAP + ads |
| V2 | Release notes v0.4.0 "Streams update" EN + AZ, dated 2026-09-17, "ready to tag", with the store "What's new" texts EN/AZ/RU/TR ≤ 500 chars (v0.3.0, v0.2.0 and v0.1.0 sections kept) | done (updated 2026-09-17 for `3e4a388` under fire, `235def1` translated level names, `577b894` screenshots) | `RELEASE_NOTES.md`; the v0.4.0 section lists only what is in the tree — "Not in this build" holds the rest; "What's new" EN/AZ/RU/TR now 489 / 485 / 497 / 493 chars |
| V3 | Git tags `tower-clash-v0.4.0` and `tower-clash-v0.3.0` (the latter exists locally only) | todo (Producer, from the GitHub UI — the git proxy accepts pushes to the working branch, not tags) | Proposed in `RELEASE_NOTES.md`; `tower-clash-v0.2.0` / `v0.1.0` can be skipped |
| V4 | Native versions aligned (see G6, A7) | done | `npm run version:check` passes at 0.4.0 / 4 (2026-09-17); can guard CI |
...
| # | Step | Status | Notes |
|---|---|---|---|
| R1 | Tutorial levels 1–3 pass with a **fresh save** under rules v2 (1 "First Taps": open a stream; 2 "Supply Line": stop a stream; 3 "Free Real Estate": let a tower fill to 25 and upgrade) | level 1 verified in CI / levels 2–3 todo (QA Engineer: one manual fresh-save pass in a browser and in the Android WebView build, note the result here) | Level 1 is covered end-to-end by `e2e/smoke.spec.ts` on a fresh browser context ("Tap your tower" → "Now tap the grey tower — the stream keeps flowing" → hint clears after the link → win, `look3-tutorial.png`); the level 2–3 step logic by `tests/ui/tutorial.test.ts`; sim and level gates by `npm run check`; `npm run playtest` 40/40 levels won at 20 seeds per commit `8e53c03`. A v0.3.0 save keeps working (schema v3; the old `sendRatio` field is ignored) — so the *fresh* save is the case to test by hand |
