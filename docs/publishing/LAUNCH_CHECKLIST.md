...
| W5 | Host `docs/publishing/PRIVACY_POLICY.md` (**v2.0**, with Part B) as a public page (copy into `dist/privacy.html` in the Pages job) | todo (QA Engineer, Pages workflow) | The URL is required by both stores; planned address `https://pervincinal.github.io/InsiderTest/privacy.html`; must be live *before* the Data safety / App Privacy forms are submitted |
...
| G17 | Privacy policy URL (policy v2.0) | todo (W5) | Planned `https://pervincinal.github.io/InsiderTest/privacy.html` |
...
| A11 | Screenshots 6.7" 1290×2796 and 6.5" 1284×2778 | done | `tower-clash/store/screenshots/apple-6.7/en/01..06.png` and `apple-6.5/en/01..06.png` |
...
| A14 | App Privacy labels: Identifiers (Device ID), Purchases, Usage Data (advertising data), Diagnostics, Coarse Location — *not linked*; **tracking: No** if MZ9 option A is adopted, **Yes** if the ATT prompt stays | needs stakeholder (submission) after MZ9 decision | Answers in `STORE_LISTING.md` §6.4; "Does this app use the IDFA?" → Yes (serve ads) |
...
| A16 | Support URL, marketing URL, privacy policy URL (policy v2.0), copyright line | marketing URL done (Pages, pending W3) / privacy URL todo (W5) / needs stakeholder (support address, copyright name) | — |
| A17 | Review notes (offline single-player, IAP via StoreKit/RevenueCat, Restore Purchases location, ATT behaviour) | done (text in `STORE_LISTING.md` §2) | — |
...
| MZ4 | **IAP review screenshot** (≥ 640×920, shows the shop with the product) uploaded per product in App Store Connect | todo (Publisher — render with `store/tools/renderStoreShots.mjs` once the shop screen exists) / needs stakeholder (upload) | One shop frame reused for all nine products is accepted; also becomes store screenshot 07 |
...
| MZ9 | **Decision: App Tracking Transparency on iOS.** Option A (recommended, `STORE_LISTING.md` §6.4): remove the `requestTrackingAuthorization()` call in `src/economy/providers/admob.ts`, label "no tracking". Option B: keep the prompt, label "Data Used to Track You" | needs Producer decision / todo (Mobile Engineer if A) | A14 depends on this |
| MZ10 | **Privacy-options entry point in Settings** (`AdMob.showPrivacyOptionsForm()` when UMP reports it is required) and a **Support ID** (RevenueCat app user id) in Settings → About, as promised in privacy policy B.2 / B.7 | todo (Frontend + Mobile Engineer) | Google requires an in-app way to re-open the consent form for EEA users who consented; the Support ID makes deletion requests possible |
...
| MZ16 | Play *Data safety* (G14), *Ads declaration* (G15), Apple *App Privacy* (A14) re-answered for the monetization build; privacy policy v2.0 live (W5) | needs stakeholder (submission) | Texts ready in `STORE_LISTING.md` §6 |
