...
Nothing in this list is a request; it is the full inventory of what stands between v0.2.0 and public availability on three channels. The stakeholder can pick up the "needs stakeholder" rows whenever they wish; everything else is already in the backlog.

Shared facts: app id / bundle id / package name **`com.pervincinal.towerclash`**, display name **Tower Clash**, version **0.2.0** (build number 2), no ads, no purchases, no data collected, no network calls. Secret names below are the exact names the CI workflows should read from *Settings → Secrets and variables → Actions*.
...
| W4 | Pages deploy workflow (`.github/workflows/tower-clash-pages.yml`: build `tower-clash/`, upload `dist/`, deploy) | done (QA Engineer) | Runs on every push touching `tower-clash/**`; the deploy job is rejected until W3 is done |
| W5 | Host `docs/publishing/PRIVACY_POLICY.md` as a public page (copy into `dist/privacy.html` in the Pages job) | todo (QA Engineer, Pages workflow) | The URL is required by both stores; planned address `https://pervincinal.github.io/InsiderTest/privacy.html` |
...
| G6 | Align native version with `package.json`: `versionName "0.2.0"`, `versionCode 2` (bump on every upload) | done (Mobile Engineer) | `npm run version:sync` (`scripts/syncVersion.mjs`) copies `version` and `config.buildNumber` from `package.json` |
...
| G9 | Feature graphic 1024×500 | done | `tower-clash/store/feature-graphic.png` (regenerated for v0.2.0) |
| G10 | Phone screenshots (≥ 2, 9:16) EN and AZ | done | `tower-clash/store/screenshots/{en,az}/01..06.png`, 1080×1920 (each < 600 KB after lossless re-encoding), v2 look, regenerated 2026-09-13 16:2x with boosters / mute / settings in the UI |
...
| G17 | Privacy policy URL | todo (W5) | Planned `https://pervincinal.github.io/InsiderTest/privacy.html` |
...
| A7 | Align `MARKETING_VERSION 0.2.0`, `CURRENT_PROJECT_VERSION 2` in `ios/App/App.xcodeproj` | done (Mobile Engineer) | Written by `npm run version:sync` |
...
| A11 | Screenshots 6.7" 1290×2796 and 6.5" 1284×2778 | done | `tower-clash/store/screenshots/apple-6.7/en/01..06.png` (1290×2796) and `apple-6.5/en/01..06.png` (1284×2778), exact sizes, each < 600 KB; `node store/tools/renderStoreShots.mjs --apple` |
...
| A15 | Export compliance: the app uses no encryption of its own and makes no network calls → not subject to export regulations | done (`ITSAppUsesNonExemptEncryption = false` in `ios/App/App/Info.plist`) / needs stakeholder (answer in App Store Connect) | — |
| A16 | Support URL, marketing URL, privacy policy URL, copyright line | marketing URL done (Pages, pending W3) / privacy URL todo (W5) / needs stakeholder (support address, copyright name) | — |
...
| # | Step | Status | Notes |
|---|---|---|---|
| V1 | `tower-clash/package.json` version `0.2.0`, `config.buildNumber` 2 | done | — |
| V2 | Release notes v0.2.0 EN + AZ | done | `RELEASE_NOTES.md`; every claim verified in the tree at capture time (see the note under the v0.2.0 heading) |
...
| V4 | Native versions aligned (see G6, A7) | done | `npm run version:sync --check` can guard CI |
