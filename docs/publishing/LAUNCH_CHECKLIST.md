...
| G10 | Phone screenshots (≥ 2, 9:16) EN and AZ | **stale since rules v2** — retake todo (Tech Artist, §7 R3) | `tower-clash/store/screenshots/{en,az}/01..07.png`, 1080×1920, are v0.3.0 renders: 02–05 show the removed SEND 100 %/50 % toggle and must not be uploaded; retake list in `STORE_LISTING.md` §1.3 (8 frames) |
...
| A11 | Screenshots 6.7" 1290×2796 and 6.5" 1284×2778 | **stale since rules v2** — retake todo (Tech Artist, §7 R3) | `tower-clash/store/screenshots/apple-6.7/en/01..07.png` and `apple-6.5/en/01..07.png` are v0.3.0 renders (02–05 show the SEND toggle); re-render with `renderStoreShots.mjs --apple` after the retake |
...
| V2 | Release notes v0.4.0 "Streams update" EN + AZ, dated 2026-09-17, "ready to tag", with the store "What's new" texts EN/AZ/RU/TR ≤ 500 chars (v0.3.0, v0.2.0 and v0.1.0 sections kept) | done | `RELEASE_NOTES.md`; the v0.4.0 section lists only what is in the tree — "Not in this build" holds the rest (rules v2.1 "Under fire" is not implemented) |
...
| R2 | Store texts updated for streams / auto-upgrade | done (Publisher, 2026-09-17) | `STORE_LISTING.md`: status paragraph, "must not be claimed" line, §1.1 / §1.2 intro + HOW IT PLAYS + settings + language lines, §1.3 screenshot table, §2 App Store screenshot note, §4 language row. Unchanged because still true: app name, short description (76 / 75), promotional text, keyword lists, ASO list, IAP table, data-safety answers. "What's new" EN/AZ/RU/TR (459 / 453 / 475 / 472 chars) in `RELEASE_NOTES.md` v0.4.0 |
| R3 | Store screenshots retaken | todo (Tech Artist) — the current files must not be uploaded | Frames 02–05 in every set (`store/screenshots/{raw,en,az,raw-apple-6.7,raw-apple-6.5,apple-6.7,apple-6.5}`) show the removed SEND 100 %/50 % toggle and, on 02, the old hint; the feature graphic (`store/feature-graphic.png`, title composition, no HUD) stays valid. Retake list with captions in `STORE_LISTING.md` §1.3: streams in both colours, a tower upgrading, the limit hint "L2 needed for 2 streams", plus re-renders of title / fortress / result / shop. `npm run build && node store/tools/renderStoreShots.mjs --all`; then update G10, A11 and the README `<img>` tags |
| R4 | Privacy policy unchanged | confirmed (Publisher re-read v2.1, 2026-09-17) | Rules v2 changes no data handling: progress and settings still live only in browser local storage / the app sandbox, no new network call, no new SDK, no new permission — policy **v2.1 stays**, no version bump, W5 / G17 / A16 unchanged. Wording follow-up only (not a data change, fold into the next policy edit together with `tower-clash/public/privacy.html`): Part A "What the game stores" and the AZ mirror list "send ratio / göndərmə nisbəti" among the saved settings — that setting no longer exists; "language" is now saved instead |
...
| R6 | README "how to play" for streams (EN + AZ) | done (Publisher, 2026-09-17) | `tower-clash/README.md`; there is no root README |
| R7 | Rules v2.1 "Under fire" is **not** described anywhere public | guard (Publisher) | Proposed in GDD §2.0, backlog RULES-2.1, not implemented at the time of writing; the working tree carried uncommitted sim/test edits toward it on 2026-09-17. If it lands before the tag, the Publisher adds it to a 0.4.x / 0.5.0 note and re-checks R1–R3 (star clocks and the tutorial lesson on level 2 change with it) |
